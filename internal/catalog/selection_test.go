...
func TestBackupRunSummariesSortByNewestPartAcrossRun(t *testing.T) {
	...
	if runs[0].Entries[0].DirectoryName != "Docs" || runs[0].Entries[1].DirectoryName != "Pics" {
		t.Fatalf("expected run entries sorted by directory name, got %#v", runs[0].Entries)
	}
}
...
func TestResolveSelectionByIDAndFullName(t *testing.T) {
	...
	index := []util.BackupEntry{
		...
		{DirectoryName: "Pics", Date: "2026-03-14", ID: util.BackupID("ABC123")},
		{DirectoryName: "Music", Date: "2026-03-13", ID: util.BackupID("ZZZ999")},
	}
	...
	if len(matchedByName) != 1 || matchedByName[0].DirectoryName != "Docs" {
...
func TestResolveSelectionByIDUsesNewestDateOnly(t *testing.T) {
	...
	index := []util.BackupEntry{
		{DirectoryName: "Docs", Date: "2026-03-14", ID: util.BackupID("ABC123")},
		{DirectoryName: "Pics", Date: "2026-03-14", ID: util.BackupID("ABC123")},
		{DirectoryName: "Old", Date: "2026-03-10", ID: util.BackupID("ABC123")},
	}
...
func TestSelectionHasDateWarningNonIDSelection(t *testing.T) {
	t.Parallel()

	index := []util.BackupEntry{
		{DirectoryName: "Docs", Date: "2026-03-14", ID: util.BackupID("ABC123")},
	}
	if SelectionHasDateWarning("all", index) {
		t.Fatal("expected no warning for non-ID selection")
	}
}

func TestSelectionHasDateWarningSingleDate(t *testing.T) {
	t.Parallel()

	index := []util.BackupEntry{
		{DirectoryName: "Docs", Date: "2026-03-14", ID: util.BackupID("ABC123")},
	}
	if SelectionHasDateWarning("ABC123", index) {
		t.Fatal("expected no warning for single-date backup ID")
	}
}

func TestSelectionHasDateWarningMultipleDates(t *testing.T) {
	t.Parallel()
	...
	if !SelectionHasDateWarning("ABC123", index) {
		t.Fatal("expected warning for multi-date backup ID")
	}
}
