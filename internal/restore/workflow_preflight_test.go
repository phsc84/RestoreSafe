...
func TestBuildRestorePreflightReportsErrors(t *testing.T) {
	...
	part := util.PartFileName(backupDir, entryWithParts.DirectoryName, entryWithParts.Date, entryWithParts.ID, 1)
	...
	items := buildRestorePreflight([]util.BackupEntry{entryWithParts, entryWithoutParts}, backupDir, restorePath)
	if len(items) != 2 {
...
func TestPrintRestorePreflightShowsYubiKeyOKAfterAuthentication(t *testing.T) {
	...
	var sb strings.Builder
	printRestorePreflightWithYubiKeyCheck(&sb, &util.Config{}, backupDir, restorePath, items, true, false, operation.LocalStagingPlan{}, func() error { return nil }, func() error { return nil })
	output := sb.String()

	authLine := "Authentication: password + YubiKey"
	okLine := "  [OK] YubiKey connected. Keep it connected before starting restore."
	authIdx := strings.Index(output, authLine)
	okIdx := strings.Index(output, okLine)
	if authIdx < 0 || okIdx < 0 {
		t.Fatalf("expected authentication and OK lines in output, got: %q", output)
	}
	if !(authIdx < okIdx) {
		t.Fatalf("expected authentication before OK line, got: %q", output)
	}
	if !strings.Contains(output, "Backup size") {
		t.Fatalf("expected backup-size line in output, got: %q", output)
	}
	if strings.Contains(output, "[WARN]") {
		t.Fatalf("did not expect any WARN when key is detected, got: %q", output)
	}
}
...
func TestPrintRestorePreflightShowsYubiKeyWarnAfterAuthentication(t *testing.T) {
	...
	var sb strings.Builder
	printRestorePreflightWithYubiKeyCheck(&sb, &util.Config{}, backupDir, restorePath, items, true, false, operation.LocalStagingPlan{}, func() error { return nil }, func() error { return errors.New("no YubiKey detected") })
	output := sb.String()

	authLine := "Authentication: password + YubiKey"
	warnLine := "  [WARN] YubiKey not connected. Remedy: Connect the YubiKey before starting restore."
	authIdx := strings.Index(output, authLine)
	warnIdx := strings.Index(output, warnLine)
	if authIdx < 0 || warnIdx < 0 {
		t.Fatalf("expected authentication and WARN lines in output, got: %q", output)
	}
	if !(authIdx < warnIdx) {
		t.Fatalf("expected authentication before WARN line, got: %q", output)
	}
	if !strings.Contains(output, "Backup size") {
		t.Fatalf("expected backup-size line in output, got: %q", output)
	}
...
func TestBuildRestorePreflightIncludesTotalSizeBytes(t *testing.T) {
	...
	items := buildRestorePreflight([]util.BackupEntry{entry}, backupDir, restorePath)
	if len(items) != 1 {
