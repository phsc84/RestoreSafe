...
func TestBuildVerifyPreflightCountsParts(t *testing.T) {
	...
	items := buildVerifyPreflight([]util.BackupEntry{fx.Entry}, fx.BackupDir)
	if len(items) != 1 {
...
func TestBuildVerifyPreflightReportsErrorForMissingParts(t *testing.T) {
	...
	items := buildVerifyPreflight([]util.BackupEntry{missing}, backupDir)
	if len(items) != 1 {
		t.Fatalf("expected 1 preflight item, got %d", len(items))
	}
...
func TestPrintVerifyPreflightShowsYubiKeyOKAfterAuthentication(t *testing.T) {
	...
	printVerifyPreflightWithYubiKeyCheck(&sb, &util.Config{}, backupDir, items, true, false, func() error { return nil }, func() error { return nil })
	output := sb.String()

	selectionLine := "  [OK] " + items[0].Entry.String() + " (parts: 1)"
	authLine := "Authentication: password + YubiKey"
	okLine := "  [OK] YubiKey connected. Keep it connected before starting verification."
	...
	if !(selectionIdx < authIdx && authIdx < okIdx) {
		t.Fatalf("expected authentication after selection, OK after authentication, got: %q", output)
	}
...
func TestPrintVerifyPreflightShowsErrorItemAndUnknownTotalSize(t *testing.T) {
	...
	var sb strings.Builder
	printVerifyPreflightWithYubiKeyCheck(&sb, &util.Config{}, backupDir, items, false, false, func() error { return nil }, func() error { return nil })
	output := sb.String()
	...
	if !strings.Contains(output, "Backup size") {
		t.Fatalf("expected backup-size line when all items have errors, got: %q", output)
	}
...
func TestPrintVerifyPreflightShowsYubiKeyWarnAfterAuthentication(t *testing.T) {
	...
	var sb strings.Builder
	printVerifyPreflightWithYubiKeyCheck(&sb, &util.Config{}, backupDir, items, true, false, func() error { return nil }, func() error { return errors.New("no YubiKey detected") })
	output := sb.String()

	authLine := "Authentication: password + YubiKey"
	warnLine := "  [WARN] YubiKey not connected. Remedy: Connect the YubiKey before starting verification."
	authIdx := strings.Index(output, authLine)
	...
	if !(authIdx < warnIdx) {
		t.Fatalf("expected WARN after authentication, got: %q", output)
	}
