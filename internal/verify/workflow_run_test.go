...
func TestRunCancelsSelectionWhenUserEntersQ(t *testing.T) {
	...
	if !strings.Contains(output, "Verification cancelled.") {
...
func TestRunReturnsErrorWhenStartPromptClosed(t *testing.T) {
	...
	// (PromptStartAction) to get EOF and return an error.
	...
	if runErr == nil {
		t.Fatal("expected error when stdin closes before start-action prompt, got nil")
	}
}
