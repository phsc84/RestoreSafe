...
// newSplitOutput builds the split writer for one source directory. When syncParts
// is true, each finalized part is flushed to disk before close; pass false when
// writing to a staging area whose parts are synced again during the move to the
// backup directory, to avoid a redundant fsync on throwaway staging files.
func newSplitOutput(backupDir, directoryName, date string, id util.BackupID, splitSizeMB int64, syncParts bool) (*util.Writer, *bufio.Writer) {
