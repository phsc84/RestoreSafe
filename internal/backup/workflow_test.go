...
func TestBackupDirectoryLogsTarBeforeEncryption(t *testing.T) {
	...
	_, backupErr := backupDirectory(sourceDir, filepath.Base(sourceDir), backupDir, "2026-03-18", util.BackupID("ORD123"), []byte("pw"), security.DefaultArgon2Params, cfg, true, logger)
	logger.Close()
	if backupErr != nil {
...
func TestBackupDirectoryLogsIODiagnosticsWhenEnabled(t *testing.T) {
	...
	_, backupErr := backupDirectory(sourceDir, filepath.Base(sourceDir), backupDir, "2026-03-18", util.BackupID("DIA999"), []byte("pw"), security.DefaultArgon2Params, cfg, true, logger)
	logger.Close()
	if backupErr != nil {
		t.Fatalf("backupDirectory failed: %v", backupErr)
	}
...
func TestBackupDirectoryLogsPartNamesAtInfoLevel(t *testing.T) {
	...
	cfg := &util.Config{SplitSizeMB: 1, IODiagnostics: false}
	_, backupErr := backupDirectory(sourceDir, filepath.Base(sourceDir), backupDir, "2026-03-18", util.BackupID("ORD124"), []byte("pw"), security.DefaultArgon2Params, cfg, true, logger)
	logger.Close()
	if backupErr != nil {
		t.Fatalf("backupDirectory failed: %v", backupErr)
	}
