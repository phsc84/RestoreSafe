...
func TestMoveBackupResultsCopiesOnlyEncryptedAndChallengeFiles(t *testing.T) {
	t.Parallel()
	...
	if err := moveBackupResults(stagingDir, backupDir, nil, nil, nil); err != nil {
		t.Fatalf("moveBackupResults returned error: %v", err)
	}
...
func TestMoveBackupResultsLogsCopyBeforeAndSummaryAfter(t *testing.T) {
	stagingDir := t.TempDir()
	backupDir := t.TempDir()
	...
	if err := moveBackupResults(stagingDir, backupDir, nil, nil, logger); err != nil {
		logger.Close()
		t.Fatalf("moveBackupResults returned error: %v", err)
	}
	logger.Close()
	...
	if !strings.Contains(logContent, "Move: [alpha]_2026-03-21_AB12CD-001.enc") {
		t.Fatalf("expected pre-copy log for part 1, got: %q", logContent)
	}
	if !strings.Contains(logContent, "Move: [alpha]_2026-03-21_AB12CD-002.enc") {
		t.Fatalf("expected pre-copy log for part 2, got: %q", logContent)
	}
	if !strings.Contains(logContent, `Moved: 2 part file(s) - [alpha] successfully moved`) {
	...
	// "Move:" lines must appear before the "Moved:" summary.
	copyIdx := strings.Index(logContent, "Move: [alpha]_2026-03-21_AB12CD-001.enc")
	summaryIdx := strings.Index(logContent, "Moved: 2 part file(s)")
	if copyIdx < 0 || summaryIdx < 0 {
		t.Fatalf("expected both Move and Moved lines, got: %q", logContent)
	}
...
func TestMoveBackupResultsLogsPerDirectoryHeaderAndSummary(t *testing.T) {
	stagingDir := t.TempDir()
	backupDir := t.TempDir()
	...
	if err := moveBackupResults(stagingDir, backupDir, nil, directorySourcePaths, logger); err != nil {
		logger.Close()
		t.Fatalf("moveBackupResults returned error: %v", err)
	}
	logger.Close()
	...
	if !strings.Contains(logContent, "Moving backup files of source directory: /data/00_Gemeinsam") {
		t.Fatalf("expected directory header for 00_Gemeinsam, got: %q", logContent)
	}
	if !strings.Contains(logContent, "Moving backup files of source directory: /data/10_Daten") {
		t.Fatalf("expected directory header for 10_Daten, got: %q", logContent)
	}
	if !strings.Contains(logContent, `Moved: 2 part file(s) - [00_Gemeinsam] successfully moved`) {
		t.Fatalf("expected completion message for 00_Gemeinsam, got: %q", logContent)
	}
	if !strings.Contains(logContent, `Moved: 1 part file(s) - [10_Daten] successfully moved`) {
		t.Fatalf("expected completion message for 10_Daten, got: %q", logContent)
	}

	// "Moved:" summary must appear after the last "Move:" of its directory.
	part2Idx := strings.Index(logContent, "Move: [00_Gemeinsam]_2026-03-21_AB12CD-002.enc")
	finishedIdx := strings.Index(logContent, `Moved: 2 part file(s) - [00_Gemeinsam] successfully moved`)
