...
// Version is the RestoreSafe application version. build.bat extracts it from
// versioninfo.json (StringFileInfo.ProductVersion) and injects it here via
// -ldflags "-X main.Version=...". The "dev" default applies to un-stamped builds
// (e.g. plain `go run`/`go build`). It is published to util.AppVersion in main().
var Version = "dev"
...
func main() {
	...
	// Publish the build-stamped version so packages can record it (e.g. the
	// first line of each backup log) without threading it through call signatures.
